define(["jquery"], function ($) {
    //    $('body').append('jQuery ' + $.fn.jquery + ' loaded!');

    var firstCell1 = [[5, 3, 0], [6, 0, 0], [0, 9, 8]];
    var secondCell1 = [[0, 7, 0], [1, 9, 5], [0, 0, 0]];
    var thirdCell1 = [[0, 0, 0], [0, 0, 0], [0, 6, 0]];
    var firstRow = [firstCell1, secondCell1, thirdCell1];

    var firstCell2 = [[8, 0, 0], [7, 0, 0], [4, 0, 0]];
    var secondCell2 = [[0, 6, 0], [8, 0, 3], [0, 2, 0]];
    var thirdCell2 = [[0, 0, 3], [0, 0, 1], [0, 0, 6]];
    var secondRow = [firstCell2, secondCell2, thirdCell2];

    var firstCell3 = [[0, 6, 0], [0, 0, 0], [0, 0, 0]];
    var secondCell3 = [[0, 0, 0], [4, 1, 9], [0, 8, 0]];
    var thirdCell3 = [[2, 8, 0], [0, 0, 5], [0, 7, 9]];
    var thirdRow = [firstCell3, secondCell3, thirdCell3];

    var board = [firstRow, secondRow, thirdRow];

    var table = $('<table></table>').addClass('sudoku-table');
    for (var i = 0; i < 3; i++) {
        var row = $('<tr></tr>').addClass('sudoku-row');
        for (var j = 0; j < 3; j++) {
            var cell = $('<td></td>').addClass('sudoku-cell');
            var cellTable = $('<table></table>').addClass('sudoku-sub-stable');
            for (var k = 0; k < 3; k++) {
                var innerRow = $('<tr></tr>').addClass('sudoku-sub-row');
                for (var l = 0; l < 3; l++) {
                    var userCell = $('<td></td>');
                    userCell.addClass('sudoku-input').text(board[i][j][k][l]);
                    userCell.bind("click", (function (i, j, k, l) {
                        return function () {
                            console.log(i, j, k, l);
                        };
                    })(i, j, k, l));
                    //(3*i+k)*10+3*j+l
                    //                    userCell.attr("id", );
                    innerRow.append(userCell);
                }
                cellTable.append(innerRow);
            }
            cell.append(cellTable);
            row.append(cell);
        }
        table.append(row);
    }
    $('.board-container').append(table);


});